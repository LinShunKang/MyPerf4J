...
public class HttpStatusClassTest {
    ...
    public void testValueOf() {
        for (int i = -100; i < 100; i++) {
            Assertions.assertEquals(UNKNOWN, HttpStatusClass.valueOf(i));
        }

        for (int i = 100; i < 200; i++) {
            Assertions.assertEquals(INFORMATIONAL, HttpStatusClass.valueOf(i));
        }

        for (int i = 200; i < 300; i++) {
            Assertions.assertEquals(SUCCESS, HttpStatusClass.valueOf(i));
        }

        for (int i = 300; i < 400; i++) {
            Assertions.assertEquals(REDIRECTION, HttpStatusClass.valueOf(i));
        }

        for (int i = 400; i < 500; i++) {
            Assertions.assertEquals(CLIENT_ERROR, HttpStatusClass.valueOf(i));
        }

        for (int i = 500; i < 600; i++) {
            Assertions.assertEquals(SERVER_ERROR, HttpStatusClass.valueOf(i));
        }
