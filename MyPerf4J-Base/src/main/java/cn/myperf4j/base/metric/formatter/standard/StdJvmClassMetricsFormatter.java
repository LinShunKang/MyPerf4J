...
public final class StdJvmClassMetricsFormatter implements TextMetricsFormatter<JvmClassMetrics> {
    ...
    private static final String TITLE_FORMAT = "MyPerf4J JVM Class Metrics [%s, %s]%n";

    private static final String DATA_TITLE_FORMAT = "%-10s%10s%10s%n";
    ...
    public String format(List<JvmClassMetrics> metricsList, long startMillis, long stopMillis) {
        ...
        try {
            sb.append(String.format(TITLE_FORMAT, formatToSeconds(startMillis), formatToSeconds(stopMillis)))
                    .append(String.format(DATA_TITLE_FORMAT, "Total", "Loaded", "Unloaded"));
