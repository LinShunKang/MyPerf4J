...
public class StdLogJvmThreadMetricsExporter extends AbstractLogJvmThreadMetricsExporter {
    ...
    private static final StdJvmThreadMetricsFormatter FORMATTER = new StdJvmThreadMetricsFormatter();
    ...
    public void process(JvmThreadMetrics metrics, long processId, long startMillis, long stopMillis) {
        final List<JvmThreadMetrics> metricsList = metricsMap.get(processId);
        if (metricsList != null) {
            metricsList.add(metrics);
        } else {
            Logger.error("StdLogJvmThreadMetricsExporter.process(" + processId + ", " + startMillis + ", "
    ...
    public void afterProcess(long processId, long startMillis, long stopMillis) {
        final List<JvmThreadMetrics> metricsList = metricsMap.remove(processId);
        if (metricsList != null) {
            logger.logAndFlush(FORMATTER.format(metricsList, startMillis, stopMillis));
        } else {
            Logger.error("StdLogJvmThreadMetricsExporter.afterProcess(" + processId + ", " + startMillis + ", "
                    + stopMillis + "): metricsList is null!!!");
        }
    }
}
