...
public class ScalableAtomicIntHashCounter implements AtomicIntHashCounter {
    ...
    private static final long IHC_OFFSET = fieldOffset(ScalableAtomicIntHashCounter.class, "ihc");

    private static final long VAL_0_OFFSET = fieldOffset(ScalableAtomicIntHashCounter.class, "val0");
    ...
    public ScalableAtomicIntHashCounter(int initialCapacity) {
    ...
    public void reset() {
        ...
        UnsafeUtils.getAndSetInt(this, VAL_0_OFFSET, 0);
    }
    ...
    private static final class IHC implements Serializable {
        ...
        private final ScalableAtomicIntHashCounter saihc;
        ...
        IHC(ScalableAtomicIntHashCounter saihc, int[] kvs) {
            this.saihc = saihc;
                ...
                if (k == key) {
                    while (true) {
                        ...
                        if (isPrime(v)) { // Simply retry from the start.
                            return copySlotAndCheck(idx, fromTableCopy).addDelta(key, delta, v != TOMB_PRIME);
                        }

                        // See if we want to move to a new table (to avoid high average re-probe
                        // counts).  We only check on the initial set of a Value from zero to not-zero
                        if (v == 0 && tableFull(reProbeTimes, len)) {
                            resize(); // Force the new table copy to start
                            return copySlotAndCheck(idx, fromTableCopy).addDelta(key, delta, fromTableCopy);
                        }
        ...
        private IHC copySlotAndCheck(int idx, boolean fromTableCopy) {
            // We're only here because the caller saw a Prime, which implies a table-copy is in progress.
            assert nextIhc != null;
            if (copySlot(idx)) {      // Copy the desired slot
                copyCheckAndPromote(1); // Record the slot copied
            }
        ...
        private boolean tableFull(int reProbeCnt, int len) {
            return
        ...
        private IHC resize() {
            ...
            newIhc = nextIhc;
            if (newIhc != null) { // See if resize is already in progress
                return newIhc; // Use the new table already
            }

            // New IHC - actually allocate the big arrays
            newIhc = new IHC(saihc, this.size, log2);
        ...
        private void helpCopy(final boolean copyAll) {
            ...
            while (copyDone < oldLen) { // Still needing to copy?
                ...
                for (int i = 0; i < MIN_COPY_WORK; i++) {
                    if (copySlot((copyIdx + i) & (oldLen - 1))) { // Made an oldtable slot go dead?
                        workDone++; // Yes!
                    }
                }
