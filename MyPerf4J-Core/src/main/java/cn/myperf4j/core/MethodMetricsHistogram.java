...
public class MethodMetricsHistogram {
    ...
    private static void recordMetrics0(int methodTagId, int tp95, int tp99, int tp999, int tp9999) {
        MethodMetricsInfo methodMetricsInfo = methodMap.get(methodTagId);
        ...
        methodMap.put(methodTagId, info);
    }
    ...
    public static void buildSysGenProfilingFile() {
        ...
        try (BufferedWriter fileWriter = new BufferedWriter(new FileWriter(tempFile, false), 8 * 1024)) {
            ...
            List<Integer> neverInvokeMethods = new ArrayList<>(128);
            ...
            for (Map.Entry<Integer, MethodMetricsInfo> entry : methodMap.entrySet()) {
                ...
                if (info.getCount() <= 0) {
                    neverInvokeMethods.add(methodId);
                    continue;
                }
            ...
            if (!neverInvokeMethods.isEmpty()) {
                fileWriter.write("#The following methods have never been invoked!\n");
                for (int i = 0; i < neverInvokeMethods.size(); i++) {
                    Integer methodId = neverInvokeMethods.get(i);
